  ...
  queries: {
    ...
    store: `SELECT 
      CASE 
        ...
        ELSE rtype 
      END AS TIPE,
        IF(A.RTYPE='K' AND A.ISTYPE='05', (SELECT KIRIM FROM TOKO),
          IF(A.RTYPE='X' AND (A.SUPCO IS NULL OR A.SUPCO=''),(SELECT KDTK FROM TOKO),IF(A.SUPCO IS NULL OR A.SUPCO='',IF(A.GUDANG IS NULL OR A.GUDANG='',(SELECT KDTK FROM TOKO),A.GUDANG),A.SUPCO))
        ) 
        AS TOKO, 
