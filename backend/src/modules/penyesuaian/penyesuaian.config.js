...
export default {
  ...
  `;
    ...
    SELECT '${cab}' AS CABANG,
           '${prd}' AS PERIODE,
           '${kdtk}' AS KDTK,
           prdcd AS PRDCD, SINGKATAN, RECID AS RECID_PRODMAST, PTAG, rp_sld_akh / saldo_akh AS BEGBAL, trfin / qty_trfin AS TRFIN, trfout / qty_trfout AS TRFOUT, sales / qty_sales AS RP_SALES, retur_sales / qty_retur_sales AS RP_RETUR_SALES, adj / qty_adj AS ADJ, ba / qty_ba AS BA, bs / qty_bs AS BS, ACOST, lcost, stock, (stock*acost) AS rp_stock,
        (stock*IF((IFNULL(qty_trfin,0)+IFNULL(qty_trfout,0)+IFNULL(qty_sales,0)+IFNULL(qty_retur_sales,0)+IFNULL(qty_adj,0)+IFNULL(qty_ba,0)+IFNULL(qty_bs,0) )= 0, rcost_flt, acost))-(rp_sld_akh+trfin-trfout-sales+retur_sales+adj+ba+bs) AS sesuai, NOW() AS updtime
    FROM (
      SELECT
        prdcd,
        prod.SINGKATAN,
      ...
      FROM (
        SELECT
          ...
          (SELECT IF(lcost=0, acost, lcost) AS lcost FROM pr_${lastday} WHERE prdcd = flt.prdcd AND toko = '${kdtk}') AS lcost,
