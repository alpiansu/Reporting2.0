...
export default {
  ...
  queries: {
    ...
    filter: (filetToko, strPeriode, month, year) => `
      SELECT (SELECT KIRIM FROM toko) AS CAB, '${strPeriode}' as PERIODE, (SELECT KDTK FROM TOKO) as KDTK, sum(sesuai) as SESUAI, NOW(), '' as KET FROM (
