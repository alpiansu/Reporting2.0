...
export const screeningByCabang = async (req, res) => {
  try {
    ...
    logger.info(`Starting screening for cabang: ${cabParam}, periode: ${periode}${isForce ? " [FORCE]" : ""}`);
 ...
 * POST /api/penyesuaian/sync-json
...
export const syncJson = async (req, res) => {
  try {
    ...
    const result = periode
      ? { periode, totalRecords: await penyesuaianService.syncToJsonFile(periode), totalFiles: 1 }
      : await penyesuaianService.syncAllData();

    return apiResponse.success(res, {
      message: "Sync JSON penyesuaian dari database berhasil",
      ...result,
    });
  } catch (error) {
    logger.error(`[penyesuaian.controller] Error syncing JSON: ${error.message}`);
    return apiResponse.error(res, error.message);
  }
};
