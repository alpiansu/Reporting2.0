...
class RekonPersediaanService {
  ...
  async syncToJsonFile() {
    try {
      ...
      const dbData = await SaldoRekonPersediaan.findAll({ where: { RECID: "*" } });
  ...
  async screening(options) {
    ...
    if (shops && shops.length > 0) {
      ...
      storesToProcess = allStores.filter(s => shops.some(code => code.toUpperCase() === (s.storeCode || s.kdtk || "").toUpperCase()))
                         .map(s => ({ ...s, cab: s.branch || s.cab }));
    } else {
      const branches = (cabang === "All" || cabang === "ALL") 
        ? [...new Set(storeService.stores.filter(s => s.notes === "INDUK").map(s => s.branch || s.cab))]
        : [cabang];
      
      const groups = await Promise.all(branches.map(cab => storeService.getStoresByBranch(cab, true).then(ss => ss.map(s => ({ ...s, cab })))));
    ...
    try {
      ...
      await Promise.all(storesToProcess.map(store => limitStores(async () => {
        ...
        try {
          ...
          if (!storeInfo) {
            processedStores++;
            if (!skipProgress) await progressService.updateProgress(taskId, processedStores, { description: `Store ${storeCode} → Info not found`, status: "Processing" });
            return;
          }

          const storeConn = await withTimeout(dbStore.createDbStore(storeInfo.dbHost, 2), 15000, `connect ${storeCode}`);
          if (!storeConn) {
            processedStores++;
            if (!skipProgress) await progressService.updateProgress(taskId, processedStores, { description: `Store ${storeCode} → Connection failed`, status: "Processing" });
            return;
          }
          ...
          try {
            for (const date of dates) {
              try {
                ...
                const h_s = [Number(sData.HPP_DRY)||0, Number(sData.HPP_ISTORE)||0, Number(sData.HPP_RESTO)||0, Number(sData.HPP_VIRTUAL)||0, Number(sData.HPP_SPC_STORE)||0];
                const h_w = [Number(wData.HPP_DRY)||0, Number(wData.HPP_ISTORE)||0, Number(wData.HPP_RESTO)||0, Number(wData.HPP_VIRTUAL)||0, Number(wData.HPP_SPC_STORE)||0];
                const diffs = h_s.map((v, i) => v - h_w[i]);
                const hasDiff = diffs.some(d => Math.abs(d) > 100);

                if (hasDiff) {
                  await SaldoRekonPersediaan.bulkCreate([{
                    CABANG: cab, SHOP: storeCode, TANGGAL: date, RECID: "*",
                    HPP_DRY_STORE: h_s[0], HPP_ISTORE_STORE: h_s[1], HPP_RESTO_STORE: h_s[2], HPP_VIRTUAL_STORE: h_s[3], HPP_SPC_STORE_STORE: h_s[4],
                    HPP_DRY_WRC: h_w[0], HPP_ISTORE_WRC: h_w[1], HPP_RESTO_WRC: h_w[2], HPP_VIRTUAL_WRC: h_w[3], HPP_SPC_STORE_WRC: h_w[4],
                    SELISIH_DRY: diffs[0], SELISIH_ISTORE: diffs[1], SELISIH_RESTO: diffs[2], SELISIH_VIRTUAL: diffs[3], SELISIH_SPC: diffs[4], LASTCATCH: new Date()
                  }], { updateOnDuplicate: ["HPP_DRY_STORE", "HPP_ISTORE_STORE", "HPP_RESTO_STORE", "HPP_VIRTUAL_STORE", "HPP_SPC_STORE_STORE", "HPP_DRY_WRC", "HPP_ISTORE_WRC", "HPP_RESTO_WRC", "HPP_VIRTUAL_WRC", "HPP_SPC_STORE_WRC", "SELISIH_DRY", "SELISIH_ISTORE", "SELISIH_RESTO", "SELISIH_VIRTUAL", "SELISIH_SPC", "RECID", "LASTCATCH"] });
                  newRecordsCount++;
                } else {
                  // Resolve if previously existed but now matches
                  await SaldoRekonPersediaan.update({ RECID: "1", LASTCATCH: new Date() }, { where: { CABANG: cab, SHOP: storeCode, TANGGAL: date, RECID: "*" } });
                }
              } catch (e) {
            ...
            processedStores++;
            if (!skipProgress) await progressService.updateProgress(taskId, processedStores, { description: `Store ${storeCode} processed`, status: "Processing" });
          } finally { await storeConn.end(); }
        } catch (err) {
          ...
          processedStores++;
          if (!skipProgress) await progressService.updateProgress(taskId, processedStores, { description: `Store ${storeCode} → Error`, status: "Processing" });
        }
      })));
      ...
      if (!skipProgress) {
          await progressService.completeProgress(taskId, { description: "All stores processed", status: "completed" });
      }

      return { success: true, message: "Screening completed", processedRecords: newRecordsCount };
    } catch (error) {
      if (!skipProgress) await progressService.failProgress(taskId, { description: error.message, status: "failed" });
      throw error;
    } finally {
  ...
  async getSummary(opts = {}) {
    ...
    const filtered = this.rekonData.filter(i => (!opts.cabang || opts.cabang === "All" || i.CABANG === opts.cabang) && (!opts.periode || moment(i.TANGGAL).format("YYMM") === opts.periode));
    return { data: { jml_toko: new Set(filtered.map(i => i.SHOP)).size, total_selisih: filtered.reduce((a, i) => a + Math.abs(i.SELISIH_DRY) + Math.abs(i.SELISIH_ISTORE) + Math.abs(i.SELISIH_RESTO) + Math.abs(i.SELISIH_VIRTUAL) + Math.abs(i.SELISIH_SPC), 0), total_records: filtered.length } };
  }

  async getAllRecords(opts = {}) {
    await this.ensureDataLoaded();
    const { page = 1, limit = 10, cabang, periode, searchQuery, sortColumn = "TANGGAL", sortOrder = "DESC" } = opts;
    let filtered = this.rekonData.filter(i => (!cabang || cabang === "All" || i.CABANG === cabang) && (!periode || moment(i.TANGGAL).format("YYMM") === periode) && (!searchQuery || i.SHOP.toLowerCase().includes(searchQuery.toLowerCase()) || i.CABANG.toLowerCase().includes(searchQuery.toLowerCase())));
    const col = ["CABANG", "SHOP", "TANGGAL", "LASTCATCH"].includes(sortColumn) ? sortColumn : "TANGGAL";
    filtered.sort((a, b) => { let vA = a[col], vB = b[col]; if (sortOrder.toUpperCase() === "DESC") [vA, vB] = [vB, vA]; return vA > vB ? 1 : vA < vB ? -1 : 0; });
    return { data: { data: filtered.slice((page - 1) * limit, page * limit), total: filtered.length, page: Number(page), limit: Number(limit), totalPages: Math.ceil(filtered.length / limit) } };
  }
}
