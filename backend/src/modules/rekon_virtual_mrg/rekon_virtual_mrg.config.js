  ...
  queries: {
    ...
    wrc: `SELECT KIRIM AS CABANG, SHOP, TANGGAL, PRDCD, SINGKATAN, ACOST, PRICE, MS_QTY AS QTY_MSTRAN, MT_QTY AS QTY_MTRAN, SEL, NOW() AS LASTCATCH FROM (
            ...
            SELECT M.SHOP, M.TANGGAL, M.PRDCD, SUM(IFNULL(IF(MT.RTYPE='J',MT.QTY,MT.QTY*-1),0)) AS MT_QTY FROM (
              ...
              SELECT STR_TO_DATE(TANGGAL,"%d-%m-%Y") AS TANGGAL, SHOP, PRDCD FROM DT_{yymmdd} WHERE PRDCD IN (SELECT DISTINCT PRDCD FROM PR_{yymmdd} WHERE CAT_COD IN ('034203','034202') AND KONS = 'K' AND SUPCO IS NOT NULL AND TOKO = '{kdtk}') AND SHOP = '{kdtk}' 
              UNION ALL
              SELECT STR_TO_DATE(TGLTRX,"%d%m%Y") AS TANGGAL, KDTOKO AS SHOP, PRDCD FROM RMB_{yymmdd} WHERE PRDCD IN (SELECT DISTINCT PRDCD FROM PR_{yymmdd} WHERE CAT_COD IN ('034203','034202') AND KONS = 'K' AND SUPCO IS NOT NULL AND TOKO = '{kdtk}') AND KDTOKO = '{kdtk}'
              ) CEK GROUP BY PRDCD
            ) M LEFT JOIN DT_{yymmdd} MT USING(SHOP, PRDCD) 
             WHERE M.SHOP = '{kdtk}' AND MT.SHOP = '{kdtk}'
            ...
            LEFT JOIN RMB_{yymmdd} MS ON M.PRDCD = MS.PRDCD AND M.SHOP = MS.KDTOKO 
            WHERE M.SHOP = '{kdtk}' AND MS.KDTOKO = '{kdtk}'
            GROUP BY M.SHOP, M.PRDCD
            ) MSTRAN 
            ...
            ) AS DT LEFT JOIN PR_{yymmdd} P USING(PRDCD) WHERE P.TOKO = '{kdtk}';`,
