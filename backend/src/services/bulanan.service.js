  ...
  async getConBulanan(cab) {
    ...
    try {
      ...
      const [rows] = await conDBEdp.execute(
        `SELECT * FROM config_cabang WHERE rkey IN('constringbulanan') AND kode_cab=?`,
        [cab]
      );
      if (!rows.length || !rows[0].nilai) {
        throw new Error("No config found for cabang: " + cab);
      }
      ...
      logger.error(`getConBulanan error: ${error.message}`);
      throw error;
    } finally {
...
module.exports = serverBulananService;
