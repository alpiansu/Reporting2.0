...
class ExternalDbService {
  constructor() {
    this.config = syncConfig.externalDb;
  ...
  async connect() {
    try {
      ...
      logger.info(`Connected to external database at ${this.config.host}`);
      return this.connection;
    } catch (error) {
      logger.error(`Failed to connect to external database: ${error.message}`);
      throw error;
    }
  }
  ...
  async fetchStoreData() {
    try {
      ...
      const query = `SELECT * FROM db_edp.rekap_ip a INNER JOIN ( 
                      SELECT * FROM ( 
                        SELECT KodeToko AS KDTK, NamaToko AS NAMATK FROM tb_toko UNION ALL 
                        SELECT Toko AS KDTK, Nama AS NAMATK FROM m_toko 
