...
class UserActivityService {
  constructor() {
    this.dataDir = path.join(__dirname, '../../data/user-activities');
  ...
  async logActivity(activityData) {
    try {
      ...
      logger.info(`Activity logged for user ${activityData.userId}: ${activityData.type} - ${activityData.description}`);
    ...
    try {
      ...
      if (startDate) {
        ...
        filteredActivities = filteredActivities.filter(activity => 
          new Date(activity.createdAt) >= startDateObj
        );
      }
      ...
      if (endDate) {
        ...
        filteredActivities = filteredActivities.filter(activity => 
          new Date(activity.createdAt) <= endDateObj
        );
      }
