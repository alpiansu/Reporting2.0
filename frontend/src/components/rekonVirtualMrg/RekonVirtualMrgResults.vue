...
import { rekonVirtualMrgService } from '../../services';
import RekonVirtualMrgTable from './RekonVirtualMrgTable.vue';
...
const loadResults = async (options = {}) => {
  ...
  try {
    ...
    // Load results
    const resultsResponse = await rekonVirtualMrgService.getDailyShopSummary(
      props.cab, 
      props.periode,
      params
    );
...
<style scoped src="./RekonVirtualMrgResults.style.css"></style>
