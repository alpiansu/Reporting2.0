...
const isFormValid = computed(() => {
  // If user selects "select" mode for PIC, they must provide a PIC
  if (picMode.value === 'select' && !pic.value) {
    return false;
  }

  // If user selects "select" mode for KDTK, they must provide at least one KDTK
...
const buildParams = () => {
  ...
  if (picMode.value === 'select' && pic.value) {
    // Extract username if pic is an object, otherwise use as is
  ...
  if (kdtkMode.value === 'select' && selectedKdtks.value.length) {
    // Extract storeCode from objects or use as is
