<template>
  <div class="rmb-history-report-card">
      ...
      <div class="header-content">
        <div class="title-section">
          ...
          <div class="text-content">
            <h2 class="card-title">Laporan History Buat RMB</h2>
            <p class="card-subtitle">Filter dan ekspor riwayat buat RMB berdasarkan periode, PIC, dan toko</p>
          </div>
        </div>
      </div>
    </div>
...
const loadFilters = async () => {
  try {
    const resp = await buatRmbService.getFilters();
...
const onPasteKdtk = (e) => {
  ...
  const newStores = list.map(code => ({ storeCode: code, storeName: '' }));
  const allStores = [...selectedKdtks.value, ...newStores];
  const uniqueStores = Array.from(new Map(allStores.map(item => [item.storeCode, item])).values());
...
const searchKdtk = (event) => {
  ...
  storeService.getAllStores({ page: 1, limit: 20, search: q })
    .then(resp => {
      ...
      const uniqueStores = Array.from(new Map(stores.map(item => [item.storeCode, item])).values());
...
const buildParams = () => {
  ...
  if (kdtkMode.value === 'select' && selectedKdtks.value.length) {
    params.kdtk = selectedKdtks.value.map(k => typeof k === 'object' ? k.storeCode : k);
  }
  return params;
};
...
const handleExport = async () => {
  if (!isFormValid.value) {
    toast.add({ severity: 'warn', summary: 'Validasi Gagal', detail: validationMessage.value, life: 3000 });
    return;
  }
  exportCsv();
};
...
const exportCsv = async () => {
  loading.value = true;
  try {
    await buatRmbService.exportHistoryCsv(buildParams());
    toast.add({ severity: 'success', summary: 'Berhasil', detail: 'File CSV berhasil diunduh', life: 3000 });
  } catch (error) {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Gagal mengekspor data. Silakan coba lagi.', life: 5000 });
...
<style scoped src="./BuatRmbHistoryReportCard.style.css"></style>
